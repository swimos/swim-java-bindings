...
public class SequenceParser {
  ...
  public static <O1, O2, O3> Parser<Pair<O1, O3>> separatedPair(Parser<O1> first, Parser<O2> sep, Parser<O3> second) {
    return input -> {
      if (input.isDone()) {
        return none(Result.incomplete(input, 1,()->separatedPair(first, sep, second)));
      } else {
  ...
  public static <O1, O2> Parser<Pair<O1, O2>> pair(Parser<O1> first, Parser<O2> second) {
    return input -> {
      if (input.isDone()) {
        return none(Result.incomplete(input, 1, ()->pair(first, second)));
      } else {
  ...
  public static <O1, O2, O3> Parser<O2> delimited(Parser<O1> first, Parser<O2> second, Parser<O3> third) {
    return input -> {
      if (input.isDone()) {
        return none(Result.incomplete(input, 1, ()->delimited(first,second,third)));
      } else {
