...
use swim_api::protocol::agent::{
    LaneRequest, LaneRequestDecoder, StoreInitMessage, StoreInitMessageDecoder, StoreInitialized,
...
impl AgentFactory {
    ...
    pub fn agent_for(&self, env: &JNIEnv, uri: impl AsRef<str>) -> Result<JavaAgentRef, Error> {
        let AgentFactory {
            new_agent_method,
            factory,
            vtable,
...
impl JavaAgentRef {
    ...
    fn init(&self, env: &JNIEnv, lane_name: &GlobalRef, msg: JByteBuffer) -> Result<(), Error> {
        let JavaAgentRef { agent_obj, vtable } = self;
        with_local_frame_null(env, None, || {
...
impl JavaAgentVTable {
    ...
    fn did_start(&self, env: &JNIEnv, agent_obj: JObject) -> Result<(), Error> {
        self.did_start.void().invoke(env, agent_obj, &[])
    }

    fn did_stop(&self, env: &JNIEnv, agent_obj: JObject) -> Result<(), Error> {
...
async fn initialize_agent(
...

    {
        ...
        for (idx, uri) in lane_specs.keys().enumerate() {
            ...
            lane_name_lookup.insert(idx as u64, uri_global_ref);
        }
    }
...
struct FfiAgentTask {
    ...
    route_params: HashMap<String, String>,
    config: AgentConfig,
    lane_name_lookup: HashMap<u64, GlobalRef>,
    java_agent: JavaAgentRef,
        ...
        loop {
            ...
            match event {
                Some(RuntimeEvent::Request { id, request }) => {
                    let lane_ref = lane_name_lookup.get(&id).cloned().expect("Missing lane");
                    handle_request(&ffi_context, &java_agent, lane_ref, request).unwrap();
                }
...
fn handle_request(
    ...
    match request {
        LaneRequest::Command(mut msg) => {
            ...
            match jni_call(&env, || agent_ref.dispatch(&env, &lane_name_ref, buffer)) {
                Ok(ret) => {
                    handle_response(context, agent_ref, ret)?;
                    Ok(())
                }
                Err(e) => panic_any(e),
            }
...
fn handle_response(
    ...
    // let mut decoder = AgentResponseDecoder::default();
    // ctx.vm  .expect_env().new_direct_byte_buffer()
...
impl Stream for LaneReader {
    type Item = (u64, Result<LaneRequest<BytesMut>, FrameIoError>);

    fn poll_next(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        let LaneReader { idx, codec } = self.get_mut();
        Pin::new(codec).poll_next(cx).map(|r| r.map(|r| (*idx, r)))
    }
}
